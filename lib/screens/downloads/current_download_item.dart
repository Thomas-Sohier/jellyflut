...
class _CurrentDownloadItemState extends State<CurrentDownloadItem> {
  ...
  void _onTap(String heroTag) {
    context.router.root.push(DetailsRoute(item: widget.ongoingDownload.item, heroTag: heroTag));
  }
        ...
        return Stack(
          ...
          children: [
            CircularProgressIndicator(
              ...
              backgroundColor: ColorUtil.lighten(Theme.of(context).colorScheme.surface),
              color: Colors.green,
            ),
            IconButton(
  ...
  Widget duration() {
    return Flexible(
      child: Text(
        printDuration(Duration(microseconds: widget.ongoingDownload.item.getDuration())),
        maxLines: 1,
