...
class _OutlinedButtonSelectorState extends State<OutlinedButtonSelector> with AbsorbAction {
  ...
  Widget build(BuildContext context) {
    return OutlinedButton(
      ...
      style: OutlinedButton.styleFrom(
        ...
        foregroundColor: primary ?? Theme.of(context).colorScheme.onSurface.withOpacity(0.05),
        alignment: alignment,
        padding: padding,
        shape: shape,
  ...
  WidgetStateProperty<BorderSide> _buttonBorderSide(BuildContext context) {
    return WidgetStateProperty.resolveWith<BorderSide>((Set<WidgetState> states) {
      if (states.contains(WidgetState.focused)) {
        return BorderSide(width: 2, color: primary ?? Theme.of(context).colorScheme.onSurface);
      }
